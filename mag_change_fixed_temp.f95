...
program Ising_Model
    ...
    integer(kind=8), parameter :: MC_max = 1E7 * latt_leng**2  
    double precision, parameter :: T = 2.269d0 ! Unit = kT/J, Tc computed with each of the physical observables
    
    call create_lattice_random 
    ...
    call flip_sites_metropolis(MC_max) ! Iterate MC simulation MC_max times per temperature
end program
...
subroutine flip_sites_metropolis(MC_max)
    use parameters
    integer :: first_open = 1
    integer :: x, y, sum_spin_nbr, sum_nbr_spin
    integer :: out_unit1 = 10, out_unit2 = 20 !,out_unit3 = 30  
    integer(kind=8), intent(in) :: MC_max
    integer(kind=8) :: MC_step
    double precision :: accept_prob, tot_en, tot_mag, tot_en2, tot_E, tot_M, tot_E2
    ...
    do MC_step = 0, MC_max
        ...
        if (MOD(MC_step, latt_leng**2) == 0) then
            if(first_open == 1) then
                open (out_unit1, FILE = 'output_ising_model_fixed_T_E_change_plot.txt', action = "write", status = 'replace')
                write (out_unit1,*) MC_step / latt_leng**2 , " ", tot_E
                close(out_unit1)
                open (out_unit2, FILE = 'output_ising_model_fixed_T_M_change_plot.txt', action = "write", status = 'replace')
                write (out_unit2,*) MC_step / latt_leng**2, " ", tot_M
                close(out_unit2)
                first_open = -1
            else 
                open (out_unit1, FILE = 'output_ising_model_fixed_T_E_change_plot.txt', action = "write", position = 'append')
                write (out_unit1,*) MC_step / latt_leng**2, " ", tot_E
                close(out_unit1)
                open (out_unit2, FILE = 'output_ising_model_fixed_T_M_change_plot.txt', action = "write", position = 'append')
                write (out_unit2,*) MC_step / latt_leng**2, " ", tot_M
                close(out_unit2)
            end if
        end if
